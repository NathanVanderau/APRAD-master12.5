...
public class SpectrogramView extends SurfaceView implements SurfaceHolder.Callback 
{
 ...
 private void drawGridTime( Canvas canvas )
 {
   ...
   /*
   canvas.drawText( "Sec", 
                    MAX_SIGNALS_LENGTH * scaleWidth + getRelativePaddingAxisX() + gridPaint.measureText("00"), 
                    scrHeight,
                    gridPaint );
   */
 }
  ...
  private void drawGridAmplitude( Canvas canvas )
  {
    ...
    if( logScaling )
    {
      drawAmplitudeValues( canvas, 
    		  			   new String[]{ "-93", "-75", "-50", "-25", "0" } );     
  ...
  {
    drawTopRecordingButton( RENDERING_RECORDING_STR, android.graphics.Color.BLACK );
    drawTopRecordingButton( STOP_RECORDING_STR, android.graphics.Color.BLACK );
  ...
  {
    drawTopRecordingButton( RENDERING_RECORDING_STR, android.graphics.Color.BLACK );
    drawTopRecordingButton( START_RECORDING_STR, android.graphics.Color.BLACK );
  ...
  private float getRelativeGridTextSize()
  {
    if( relativeGridTextSize == null ){
      relativeGridTextSize = TypedValue.applyDimension( 
